...
export type Review = {
  ...
  countryCode?: string; // Optional "RO", "HU", etc. (renders a tiny flag emoji)
};
...
type Props = {
  ...
  profileUrl?: string;     // link to your (mock) profile page
};
...
export default function ReviewCarousel({
  ...
  }, [autoPlayMs, count, prefersReduced]);
  ...
  return (
    <section
      ref={containerRef}
      className={`relative w-full max-w-3xl mx-auto rounded-2xl shadow-lg bg-white p-0 overflow-hidden ${className ?? ""}`}
      role="region"
    ...
    >
      ...
      <div className="relative h-full">
        <div
          key={current.id}
          // Slide / fade combo; reduced motion = no animation
          className={[
            "px-6 md:px-10 py-7 md:py-10",
            prefersReduced
              ? ""
              : "animate-[slideIn_300ms_ease-out] will-change-transform",
            dir === 1 ? "origin-right" : "origin-left",
          ].join(" ")}
          style={{
            // CSS keyframes in JS to keep this self-contained:
            // @keyframes slideIn { from { opacity:.01; transform: translateX(var(--x, 12px)); } to { opacity:1; transform: translateX(0); } }
            // We'll emulate via inline custom property:
            ["--x" as any]: dir === 1 ? "16px" : "-16px",
          }}
        >
          {/* Stars (emerald like Trustpilot) */}
          <div className="flex justify-center mb-3" aria-label={`${current.rating} out of 5 stars`}>
            {Array.from({ length: 5 }).map((_, i) => (
              <Star
                key={i}
                className={`w-5 h-5 ${
                  i < current.rating ? "text-emerald-500 fill-emerald-500" : "text-neutral-300"
                }`}
                aria-hidden
              />
            ))}
          ...
          <div className="mt-4 text-center text-xs text-neutral-500">
            ...
            {current.date ? <span> · {formatDate(current.date)}</span> : null}
            {" "}
            {current.countryCode ? <span>· {flagEmoji(current.countryCode)}</span> : null}
          </div>
      ...
      <div className="flex items-center justify-between px-4 pb-4 pt-1">
        ...
        <div className="flex items-center gap-2">
          {/* Dots */}
          <div className="flex gap-1.5" role="tablist" aria-label="Review selector">
            {reviews.map((_, i) => (
              <button
                key={i}
                onClick={() => {
                ...
                className={`h-2.5 w-2.5 rounded-full transition ${
                  i === index ? "bg-emerald-600" : "bg-neutral-300 hover:bg-neutral-400"
                }`}
              />
            ))}
          </div>
      ...
      <style jsx>{`
        @keyframes slideIn {
          from { opacity: .01; transform: translateX(var(--x, 12px)); }
...
/** Small pill: TrustScore 4.2 | 27 reviews */
function TrustScoreBadge({ trustScore, totalCount }: { trustScore: number; totalCount: number }) {
  ...
  return (
    <span className="inline-flex items-center gap-2 rounded-full bg-white border px-2.5 py-1 shadow-sm hover:shadow transition">
      <span className="flex items-center gap-1" aria-label={`TrustScore ${rounded} out of 5`}>
        {/* 5 green stars */}
        {Array.from({ length: 5 }).map((_, i) => (
          <Star
            key={i}
            className={`w-3.5 h-3.5 ${
              i < Math.round(rounded) ? "text-emerald-500 fill-emerald-500" : "text-neutral-300"
            }`}
            aria-hidden
          />
        ))}
...
function formatDate(d: string) {
  // Accepts ISO or plain; falls back to as-is
  const date = new Date(d);
  return isNaN(date.getTime()) ? d : date.toLocaleDateString(undefined, {
    year: "numeric",
    month: "short",
    day: "numeric",
  });
